...
expName = 'experiment_part1'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\labra\\Documents\\gitlab\\perceptual_attention_learning\\experiment_perceptual_learning part_1_lastrun.py',
...
text_break_stat = visual.TextStim(win=win, name='text_break_stat',
    text='',
    font='Open Sans',
...
while continueRoutine:
    ...
    waitOnFlip = False
    if key_resp_intro.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
        # keep track of start time/frame for later
...
training = data.TrialHandler(nReps=100.0, method='sequential', 
...
for thisTraining in training:
    ...
    # set up handler to look after randomisation of conditions etc
    training_trials = data.TrialHandler(nReps=trials_in_block, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=[None],
    ...
    for thisTraining_trial in training_trials:
        ...
        for thisComponent in fixation_pointComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        event.clearEvents()
        training_trials.addData('noise.started', noise.tStartRefresh)
        training_trials.addData('noise.stopped', noise.tStopRefresh)
        training_trials.addData('fix_point1.started', fix_point1.tStartRefresh)
        training_trials.addData('fix_point1.stopped', fix_point1.tStopRefresh)
        training_trials.addData('fix_point2.started', fix_point2.tStartRefresh)
        training_trials.addData('fix_point2.stopped', fix_point2.tStopRefresh)
        ...
        training_trials.addData('sound_trial.started', sound_trial.tStartRefresh)
        training_trials.addData('sound_trial.stopped', sound_trial.tStopRefresh)
        training_trials.addData('grating_trial.started', grating_trial.tStartRefresh)
        training_trials.addData('grating_trial.stopped', grating_trial.tStopRefresh)
        # check responses
        if key_resp_trial.keys in ['', [], None]:  # No response was made
            key_resp_trial.keys = None
        training_trials.addData('key_resp_trial.keys',key_resp_trial.keys)
        if key_resp_trial.keys != None:  # we had a response
            training_trials.addData('key_resp_trial.rt', key_resp_trial.rt)
        training_trials.addData('key_resp_trial.started', key_resp_trial.tStartRefresh)
        training_trials.addData('key_resp_trial.stopped', key_resp_trial.tStopRefresh)
        ...
        event.clearEvents()
        training_trials.addData('noise_3.started', noise_3.tStartRefresh)
        training_trials.addData('noise_3.stopped', noise_3.tStopRefresh)
        training_trials.addData('correct_green_2.started', correct_green_2.tStartRefresh)
        training_trials.addData('correct_green_2.stopped', correct_green_2.tStopRefresh)
        training_trials.addData('incorrect_red_2.started', incorrect_red_2.tStartRefresh)
        training_trials.addData('incorrect_red_2.stopped', incorrect_red_2.tStopRefresh)
        # the Routine "response_feedback" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
        
    # completed trials_in_block repeats of 'training_trials'
    ...
    logging.data("sum performance records: " + str(performance_record))
    ...
    text_break_stat.setText(stat_text)
    polygon_2.setPos((x, -0.3))
    polygon_2.setSize((size, 0.05))
    # keep track of which components have finished
    brkComponents = [press_space_break, key_resp_break, text_break_stat, BREAK, polygon, polygon_2]
    ...
    while continueRoutine:
        ...
        # *text_break_stat* updates
        if text_break_stat.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            text_break_stat.frameNStart = frameN  # exact frame index
            text_break_stat.tStart = t  # local t and not account for scr refresh
            text_break_stat.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(text_break_stat, 'tStartRefresh')  # time at next scr refresh
            text_break_stat.setAutoDraw(True)
    ...
    logging.data("utcTime: " + str(time.time()))
    event.clearEvents()
    training.addData('press_space_break.started', press_space_break.tStartRefresh)
    training.addData('press_space_break.stopped', press_space_break.tStopRefresh)
    # check responses
    if key_resp_break.keys in ['', [], None]:  # No response was made
        key_resp_break.keys = None
    training.addData('key_resp_break.keys',key_resp_break.keys)
    if key_resp_break.keys != None:  # we had a response
        training.addData('key_resp_break.rt', key_resp_break.rt)
    training.addData('key_resp_break.started', key_resp_break.tStartRefresh)
    training.addData('key_resp_break.stopped', key_resp_break.tStopRefresh)
    training.addData('text_break_stat.started', text_break_stat.tStartRefresh)
    training.addData('text_break_stat.stopped', text_break_stat.tStopRefresh)
    training.addData('BREAK.started', BREAK.tStartRefresh)
    training.addData('BREAK.stopped', BREAK.tStopRefresh)
    training.addData('polygon.started', polygon.tStartRefresh)
    training.addData('polygon.stopped', polygon.tStopRefresh)
    training.addData('polygon_2.started', polygon_2.tStartRefresh)
    training.addData('polygon_2.stopped', polygon_2.tStopRefresh)
    # the Routine "brk" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
    
# completed 100.0 repeats of 'training'
...
# set up handler to look after randomisation of conditions etc
ids1 = data.TrialHandler(nReps=100.0, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=[None],
    seed=None, name='ids1')
thisExp.addLoop(ids1)  # add the loop to the experiment
thisIds1 = ids1.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisIds1.rgb)
if thisIds1 != None:
    for paramName in thisIds1:
        exec('{} = thisIds1[paramName]'.format(paramName))

for thisIds1 in ids1:
    currentLoop = ids1
    # abbreviate parameter names if possible (e.g. rgb = thisIds1.rgb)
    if thisIds1 != None:
        for paramName in thisIds1:
    ...
    # set up handler to look after randomisation of conditions etc
    trials_ids1 = data.TrialHandler(nReps=trials_in_block, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=[None],
        seed=None, name='trials_ids1')
    thisExp.addLoop(trials_ids1)  # add the loop to the experiment
    thisTrials_ids1 = trials_ids1.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisTrials_ids1.rgb)
    if thisTrials_ids1 != None:
        for paramName in thisTrials_ids1:
            exec('{} = thisTrials_ids1[paramName]'.format(paramName))
    
    for thisTrials_ids1 in trials_ids1:
        currentLoop = trials_ids1
        # abbreviate parameter names if possible (e.g. rgb = thisTrials_ids1.rgb)
        if thisTrials_ids1 != None:
            for paramName in thisTrials_ids1:
                exec('{} = thisTrials_ids1[paramName]'.format(paramName))
        ...
        for thisComponent in fixation_pointComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        event.clearEvents()
        trials_ids1.addData('noise.started', noise.tStartRefresh)
        trials_ids1.addData('noise.stopped', noise.tStopRefresh)
        trials_ids1.addData('fix_point1.started', fix_point1.tStartRefresh)
        trials_ids1.addData('fix_point1.stopped', fix_point1.tStopRefresh)
        trials_ids1.addData('fix_point2.started', fix_point2.tStartRefresh)
        trials_ids1.addData('fix_point2.stopped', fix_point2.tStopRefresh)
        ...
        sound_trial.stop()  # ensure sound has stopped at end of routine
        trials_ids1.addData('sound_trial.started', sound_trial.tStartRefresh)
        trials_ids1.addData('sound_trial.stopped', sound_trial.tStopRefresh)
        trials_ids1.addData('grating_trial.started', grating_trial.tStartRefresh)
        trials_ids1.addData('grating_trial.stopped', grating_trial.tStopRefresh)
        # check responses
        if key_resp_trial.keys in ['', [], None]:  # No response was made
            key_resp_trial.keys = None
        trials_ids1.addData('key_resp_trial.keys',key_resp_trial.keys)
        if key_resp_trial.keys != None:  # we had a response
            trials_ids1.addData('key_resp_trial.rt', key_resp_trial.rt)
        trials_ids1.addData('key_resp_trial.started', key_resp_trial.tStartRefresh)
        trials_ids1.addData('key_resp_trial.stopped', key_resp_trial.tStopRefresh)
        ...
        event.clearEvents()
        trials_ids1.addData('noise_3.started', noise_3.tStartRefresh)
        trials_ids1.addData('noise_3.stopped', noise_3.tStopRefresh)
        trials_ids1.addData('correct_green_2.started', correct_green_2.tStartRefresh)
        trials_ids1.addData('correct_green_2.stopped', correct_green_2.tStopRefresh)
        trials_ids1.addData('incorrect_red_2.started', incorrect_red_2.tStartRefresh)
        trials_ids1.addData('incorrect_red_2.stopped', incorrect_red_2.tStopRefresh)
        # the Routine "response_feedback" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
        
    # completed trials_in_block repeats of 'trials_ids1'
    ...
    logging.data("utcTime: " + str(time.time()))
    event.clearEvents()
    ids1.addData('press_space_break.started', press_space_break.tStartRefresh)
    ids1.addData('press_space_break.stopped', press_space_break.tStopRefresh)
    # check responses
    if key_resp_break.keys in ['', [], None]:  # No response was made
        key_resp_break.keys = None
    ids1.addData('key_resp_break.keys',key_resp_break.keys)
    if key_resp_break.keys != None:  # we had a response
        ids1.addData('key_resp_break.rt', key_resp_break.rt)
    ids1.addData('key_resp_break.started', key_resp_break.tStartRefresh)
    ids1.addData('key_resp_break.stopped', key_resp_break.tStopRefresh)
    ids1.addData('text_break_stat.started', text_break_stat.tStartRefresh)
    ids1.addData('text_break_stat.stopped', text_break_stat.tStopRefresh)
    ids1.addData('BREAK.started', BREAK.tStartRefresh)
    ids1.addData('BREAK.stopped', BREAK.tStopRefresh)
    ids1.addData('polygon.started', polygon.tStartRefresh)
    ids1.addData('polygon.stopped', polygon.tStopRefresh)
    ids1.addData('polygon_2.started', polygon_2.tStartRefresh)
    ids1.addData('polygon_2.stopped', polygon_2.tStopRefresh)
    # the Routine "brk" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
    
# completed 100.0 repeats of 'ids1'
