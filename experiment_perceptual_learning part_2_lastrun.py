...
stages = ['previous_part',"staircase1", "ams1", "eds1", "ids2"]
stage_number = 0
...
index = 1 #index for the stimuli file
...
expName = 'experiment_part2'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\labra\\Documents\\gitlab\\perceptual_attention_learning\\experiment_perceptual_learning part_2_lastrun.py',
...
# Initialize components for Routine "end_break"
end_breakClock = core.Clock()
timer = core.Clock()
text_7 = visual.TextStim(win=win, name='text_7',
    text='Please, take 3 minutes break.\nWe would kindly ask you NOT to stay in your seat, but do some light activity: stretching or walking. ',
    font='Open Sans',
...
text_8 = visual.TextStim(win=win, name='text_8',
    text='Please, come back to your seat and wait for the experimenter to begin callibration procedure. ',
    font='Open Sans',
    pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
...
polygon_3 = visual.Rect(
    ...
    width=(0.5, 0.07)[0], height=(0.5, 0.07)[1],
    ori=0.0, pos=(0, 0.3),
    lineWidth=3.0,     colorSpace='rgb',  lineColor='white', fillColor='grey',
    opacity=None, depth=-4.0, interpolate=True)
...
staircase1 = data.TrialHandler(nReps=100.0, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=[None],
    seed=None, name='staircase1')
thisExp.addLoop(staircase1)  # add the loop to the experiment
thisStaircase1 = staircase1.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisStaircase1.rgb)
if thisStaircase1 != None:
    for paramName in thisStaircase1:
        exec('{} = thisStaircase1[paramName]'.format(paramName))

for thisStaircase1 in staircase1:
    currentLoop = staircase1
    # abbreviate parameter names if possible (e.g. rgb = thisStaircase1.rgb)
    if thisStaircase1 != None:
        for paramName in thisStaircase1:
            exec('{} = thisStaircase1[paramName]'.format(paramName))
    ...
    # set up handler to look after randomisation of conditions etc
    trials_staircase1 = data.TrialHandler(nReps=trials_in_block, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=[None],
        seed=None, name='trials_staircase1')
    thisExp.addLoop(trials_staircase1)  # add the loop to the experiment
    thisTrials_staircase1 = trials_staircase1.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisTrials_staircase1.rgb)
    if thisTrials_staircase1 != None:
        for paramName in thisTrials_staircase1:
            exec('{} = thisTrials_staircase1[paramName]'.format(paramName))
    
    for thisTrials_staircase1 in trials_staircase1:
        currentLoop = trials_staircase1
        # abbreviate parameter names if possible (e.g. rgb = thisTrials_staircase1.rgb)
        if thisTrials_staircase1 != None:
            for paramName in thisTrials_staircase1:
                exec('{} = thisTrials_staircase1[paramName]'.format(paramName))
        ...
        for thisComponent in fixation_pointComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        event.clearEvents()
        trials_staircase1.addData('noise.started', noise.tStartRefresh)
        trials_staircase1.addData('noise.stopped', noise.tStopRefresh)
        trials_staircase1.addData('fix_point1.started', fix_point1.tStartRefresh)
        trials_staircase1.addData('fix_point1.stopped', fix_point1.tStopRefresh)
        trials_staircase1.addData('fix_point2.started', fix_point2.tStartRefresh)
        trials_staircase1.addData('fix_point2.stopped', fix_point2.tStopRefresh)
        ...
        trials_staircase1.addData('sound_trial.started', sound_trial.tStartRefresh)
        trials_staircase1.addData('sound_trial.stopped', sound_trial.tStopRefresh)
        trials_staircase1.addData('grating_trial.started', grating_trial.tStartRefresh)
        trials_staircase1.addData('grating_trial.stopped', grating_trial.tStopRefresh)
        # check responses
        if key_resp_trial.keys in ['', [], None]:  # No response was made
            key_resp_trial.keys = None
        trials_staircase1.addData('key_resp_trial.keys',key_resp_trial.keys)
        if key_resp_trial.keys != None:  # we had a response
            trials_staircase1.addData('key_resp_trial.rt', key_resp_trial.rt)
        trials_staircase1.addData('key_resp_trial.started', key_resp_trial.tStartRefresh)
        trials_staircase1.addData('key_resp_trial.stopped', key_resp_trial.tStopRefresh)
        ...
        event.clearEvents()
        trials_staircase1.addData('noise_3.started', noise_3.tStartRefresh)
        trials_staircase1.addData('noise_3.stopped', noise_3.tStopRefresh)
        trials_staircase1.addData('correct_green_2.started', correct_green_2.tStartRefresh)
        trials_staircase1.addData('correct_green_2.stopped', correct_green_2.tStopRefresh)
        trials_staircase1.addData('incorrect_red_2.started', incorrect_red_2.tStartRefresh)
        trials_staircase1.addData('incorrect_red_2.stopped', incorrect_red_2.tStopRefresh)
        # the Routine "response_feedback" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
        
    # completed trials_in_block repeats of 'trials_staircase1'
    ...
    logging.data("utcTime: " + str(time.time()))
    event.clearEvents()
    staircase1.addData('press_space_break.started', press_space_break.tStartRefresh)
    staircase1.addData('press_space_break.stopped', press_space_break.tStopRefresh)
    # check responses
    if key_resp_break.keys in ['', [], None]:  # No response was made
        key_resp_break.keys = None
    staircase1.addData('key_resp_break.keys',key_resp_break.keys)
    if key_resp_break.keys != None:  # we had a response
        staircase1.addData('key_resp_break.rt', key_resp_break.rt)
    staircase1.addData('key_resp_break.started', key_resp_break.tStartRefresh)
    staircase1.addData('key_resp_break.stopped', key_resp_break.tStopRefresh)
    staircase1.addData('BREAK.started', BREAK.tStartRefresh)
    staircase1.addData('BREAK.stopped', BREAK.tStopRefresh)
    staircase1.addData('polygon.started', polygon.tStartRefresh)
    staircase1.addData('polygon.stopped', polygon.tStopRefresh)
    staircase1.addData('polygon_2.started', polygon_2.tStartRefresh)
    staircase1.addData('polygon_2.stopped', polygon_2.tStopRefresh)
    # the Routine "brk" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
    
# completed 100.0 repeats of 'staircase1'
...
# set up handler to look after randomisation of conditions etc
ams1 = data.TrialHandler(nReps=2.0, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=[None],
    seed=None, name='ams1')
thisExp.addLoop(ams1)  # add the loop to the experiment
thisAms1 = ams1.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisAms1.rgb)
if thisAms1 != None:
    for paramName in thisAms1:
        exec('{} = thisAms1[paramName]'.format(paramName))

for thisAms1 in ams1:
    currentLoop = ams1
    # abbreviate parameter names if possible (e.g. rgb = thisAms1.rgb)
    if thisAms1 != None:
        for paramName in thisAms1:
    ...
    # set up handler to look after randomisation of conditions etc
    trials_ams1 = data.TrialHandler(nReps=trials_in_block, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=[None],
        seed=None, name='trials_ams1')
    thisExp.addLoop(trials_ams1)  # add the loop to the experiment
    thisTrials_ams1 = trials_ams1.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisTrials_ams1.rgb)
    if thisTrials_ams1 != None:
        for paramName in thisTrials_ams1:
            exec('{} = thisTrials_ams1[paramName]'.format(paramName))
    
    for thisTrials_ams1 in trials_ams1:
        currentLoop = trials_ams1
        # abbreviate parameter names if possible (e.g. rgb = thisTrials_ams1.rgb)
        if thisTrials_ams1 != None:
            for paramName in thisTrials_ams1:
                exec('{} = thisTrials_ams1[paramName]'.format(paramName))
        ...
        for thisComponent in fixation_pointComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        event.clearEvents()
        trials_ams1.addData('noise.started', noise.tStartRefresh)
        trials_ams1.addData('noise.stopped', noise.tStopRefresh)
        trials_ams1.addData('fix_point1.started', fix_point1.tStartRefresh)
        trials_ams1.addData('fix_point1.stopped', fix_point1.tStopRefresh)
        trials_ams1.addData('fix_point2.started', fix_point2.tStartRefresh)
        trials_ams1.addData('fix_point2.stopped', fix_point2.tStopRefresh)
        ...
        sound_trial.stop()  # ensure sound has stopped at end of routine
        trials_ams1.addData('sound_trial.started', sound_trial.tStartRefresh)
        trials_ams1.addData('sound_trial.stopped', sound_trial.tStopRefresh)
        trials_ams1.addData('grating_trial.started', grating_trial.tStartRefresh)
        trials_ams1.addData('grating_trial.stopped', grating_trial.tStopRefresh)
        # check responses
        if key_resp_trial.keys in ['', [], None]:  # No response was made
            key_resp_trial.keys = None
        trials_ams1.addData('key_resp_trial.keys',key_resp_trial.keys)
        if key_resp_trial.keys != None:  # we had a response
            trials_ams1.addData('key_resp_trial.rt', key_resp_trial.rt)
        trials_ams1.addData('key_resp_trial.started', key_resp_trial.tStartRefresh)
        trials_ams1.addData('key_resp_trial.stopped', key_resp_trial.tStopRefresh)
        ...
        event.clearEvents()
        trials_ams1.addData('noise_3.started', noise_3.tStartRefresh)
        trials_ams1.addData('noise_3.stopped', noise_3.tStopRefresh)
        trials_ams1.addData('correct_green_2.started', correct_green_2.tStartRefresh)
        trials_ams1.addData('correct_green_2.stopped', correct_green_2.tStopRefresh)
        trials_ams1.addData('incorrect_red_2.started', incorrect_red_2.tStartRefresh)
        trials_ams1.addData('incorrect_red_2.stopped', incorrect_red_2.tStopRefresh)
        # the Routine "response_feedback" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
        
    # completed trials_in_block repeats of 'trials_ams1'
    ...
    logging.data("utcTime: " + str(time.time()))
    event.clearEvents()
    ams1.addData('press_space_break.started', press_space_break.tStartRefresh)
    ams1.addData('press_space_break.stopped', press_space_break.tStopRefresh)
    # check responses
    if key_resp_break.keys in ['', [], None]:  # No response was made
        key_resp_break.keys = None
    ams1.addData('key_resp_break.keys',key_resp_break.keys)
    if key_resp_break.keys != None:  # we had a response
        ams1.addData('key_resp_break.rt', key_resp_break.rt)
    ams1.addData('key_resp_break.started', key_resp_break.tStartRefresh)
    ams1.addData('key_resp_break.stopped', key_resp_break.tStopRefresh)
    ams1.addData('BREAK.started', BREAK.tStartRefresh)
    ams1.addData('BREAK.stopped', BREAK.tStopRefresh)
    ams1.addData('polygon.started', polygon.tStartRefresh)
    ams1.addData('polygon.stopped', polygon.tStopRefresh)
    ams1.addData('polygon_2.started', polygon_2.tStartRefresh)
    ams1.addData('polygon_2.stopped', polygon_2.tStopRefresh)
    # the Routine "brk" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
    
# completed 2.0 repeats of 'ams1'
...
# set up handler to look after randomisation of conditions etc
eds1 = data.TrialHandler(nReps=100.0, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=[None],
    seed=None, name='eds1')
thisExp.addLoop(eds1)  # add the loop to the experiment
thisEds1 = eds1.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisEds1.rgb)
if thisEds1 != None:
    for paramName in thisEds1:
        exec('{} = thisEds1[paramName]'.format(paramName))

for thisEds1 in eds1:
    currentLoop = eds1
    # abbreviate parameter names if possible (e.g. rgb = thisEds1.rgb)
    if thisEds1 != None:
        for paramName in thisEds1:
    ...
    # set up handler to look after randomisation of conditions etc
    trials_eds1 = data.TrialHandler(nReps=trials_in_block, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=[None],
        seed=None, name='trials_eds1')
    thisExp.addLoop(trials_eds1)  # add the loop to the experiment
    thisTrials_eds1 = trials_eds1.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisTrials_eds1.rgb)
    if thisTrials_eds1 != None:
        for paramName in thisTrials_eds1:
            exec('{} = thisTrials_eds1[paramName]'.format(paramName))
    
    for thisTrials_eds1 in trials_eds1:
        currentLoop = trials_eds1
        # abbreviate parameter names if possible (e.g. rgb = thisTrials_eds1.rgb)
        if thisTrials_eds1 != None:
            for paramName in thisTrials_eds1:
                exec('{} = thisTrials_eds1[paramName]'.format(paramName))
        ...
        for thisComponent in fixation_pointComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        event.clearEvents()
        trials_eds1.addData('noise.started', noise.tStartRefresh)
        trials_eds1.addData('noise.stopped', noise.tStopRefresh)
        trials_eds1.addData('fix_point1.started', fix_point1.tStartRefresh)
        trials_eds1.addData('fix_point1.stopped', fix_point1.tStopRefresh)
        trials_eds1.addData('fix_point2.started', fix_point2.tStartRefresh)
        trials_eds1.addData('fix_point2.stopped', fix_point2.tStopRefresh)
        ...
        sound_trial.stop()  # ensure sound has stopped at end of routine
        trials_eds1.addData('sound_trial.started', sound_trial.tStartRefresh)
        trials_eds1.addData('sound_trial.stopped', sound_trial.tStopRefresh)
        trials_eds1.addData('grating_trial.started', grating_trial.tStartRefresh)
        trials_eds1.addData('grating_trial.stopped', grating_trial.tStopRefresh)
        # check responses
        if key_resp_trial.keys in ['', [], None]:  # No response was made
            key_resp_trial.keys = None
        trials_eds1.addData('key_resp_trial.keys',key_resp_trial.keys)
        if key_resp_trial.keys != None:  # we had a response
            trials_eds1.addData('key_resp_trial.rt', key_resp_trial.rt)
        trials_eds1.addData('key_resp_trial.started', key_resp_trial.tStartRefresh)
        trials_eds1.addData('key_resp_trial.stopped', key_resp_trial.tStopRefresh)
        ...
        event.clearEvents()
        trials_eds1.addData('noise_3.started', noise_3.tStartRefresh)
        trials_eds1.addData('noise_3.stopped', noise_3.tStopRefresh)
        trials_eds1.addData('correct_green_2.started', correct_green_2.tStartRefresh)
        trials_eds1.addData('correct_green_2.stopped', correct_green_2.tStopRefresh)
        trials_eds1.addData('incorrect_red_2.started', incorrect_red_2.tStartRefresh)
        trials_eds1.addData('incorrect_red_2.stopped', incorrect_red_2.tStopRefresh)
        # the Routine "response_feedback" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
        
    # completed trials_in_block repeats of 'trials_eds1'
    ...
    logging.data("utcTime: " + str(time.time()))
    event.clearEvents()
    eds1.addData('press_space_break.started', press_space_break.tStartRefresh)
    eds1.addData('press_space_break.stopped', press_space_break.tStopRefresh)
    # check responses
    if key_resp_break.keys in ['', [], None]:  # No response was made
        key_resp_break.keys = None
    eds1.addData('key_resp_break.keys',key_resp_break.keys)
    if key_resp_break.keys != None:  # we had a response
        eds1.addData('key_resp_break.rt', key_resp_break.rt)
    eds1.addData('key_resp_break.started', key_resp_break.tStartRefresh)
    eds1.addData('key_resp_break.stopped', key_resp_break.tStopRefresh)
    eds1.addData('BREAK.started', BREAK.tStartRefresh)
    eds1.addData('BREAK.stopped', BREAK.tStopRefresh)
    eds1.addData('polygon.started', polygon.tStartRefresh)
    eds1.addData('polygon.stopped', polygon.tStopRefresh)
    eds1.addData('polygon_2.started', polygon_2.tStartRefresh)
    eds1.addData('polygon_2.stopped', polygon_2.tStopRefresh)
    # the Routine "brk" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
    
# completed 100.0 repeats of 'eds1'
...
# set up handler to look after randomisation of conditions etc
ids2 = data.TrialHandler(nReps=100.0, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=[None],
    seed=None, name='ids2')
thisExp.addLoop(ids2)  # add the loop to the experiment
thisIds2 = ids2.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisIds2.rgb)
if thisIds2 != None:
    for paramName in thisIds2:
        exec('{} = thisIds2[paramName]'.format(paramName))

for thisIds2 in ids2:
    currentLoop = ids2
    # abbreviate parameter names if possible (e.g. rgb = thisIds2.rgb)
    if thisIds2 != None:
        for paramName in thisIds2:
    ...
    # set up handler to look after randomisation of conditions etc
    trials_ids2 = data.TrialHandler(nReps=trials_in_block, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=[None],
        seed=None, name='trials_ids2')
    thisExp.addLoop(trials_ids2)  # add the loop to the experiment
    thisTrials_ids2 = trials_ids2.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisTrials_ids2.rgb)
    if thisTrials_ids2 != None:
        for paramName in thisTrials_ids2:
            exec('{} = thisTrials_ids2[paramName]'.format(paramName))
    
    for thisTrials_ids2 in trials_ids2:
        currentLoop = trials_ids2
        # abbreviate parameter names if possible (e.g. rgb = thisTrials_ids2.rgb)
        if thisTrials_ids2 != None:
            for paramName in thisTrials_ids2:
                exec('{} = thisTrials_ids2[paramName]'.format(paramName))
        ...
        for thisComponent in fixation_pointComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        event.clearEvents()
        trials_ids2.addData('noise.started', noise.tStartRefresh)
        trials_ids2.addData('noise.stopped', noise.tStopRefresh)
        trials_ids2.addData('fix_point1.started', fix_point1.tStartRefresh)
        trials_ids2.addData('fix_point1.stopped', fix_point1.tStopRefresh)
        trials_ids2.addData('fix_point2.started', fix_point2.tStartRefresh)
        trials_ids2.addData('fix_point2.stopped', fix_point2.tStopRefresh)
        ...
        sound_trial.stop()  # ensure sound has stopped at end of routine
        trials_ids2.addData('sound_trial.started', sound_trial.tStartRefresh)
        trials_ids2.addData('sound_trial.stopped', sound_trial.tStopRefresh)
        trials_ids2.addData('grating_trial.started', grating_trial.tStartRefresh)
        trials_ids2.addData('grating_trial.stopped', grating_trial.tStopRefresh)
        # check responses
        if key_resp_trial.keys in ['', [], None]:  # No response was made
            key_resp_trial.keys = None
        trials_ids2.addData('key_resp_trial.keys',key_resp_trial.keys)
        if key_resp_trial.keys != None:  # we had a response
            trials_ids2.addData('key_resp_trial.rt', key_resp_trial.rt)
        trials_ids2.addData('key_resp_trial.started', key_resp_trial.tStartRefresh)
        trials_ids2.addData('key_resp_trial.stopped', key_resp_trial.tStopRefresh)
        ...
        event.clearEvents()
        trials_ids2.addData('noise_3.started', noise_3.tStartRefresh)
        trials_ids2.addData('noise_3.stopped', noise_3.tStopRefresh)
        trials_ids2.addData('correct_green_2.started', correct_green_2.tStartRefresh)
        trials_ids2.addData('correct_green_2.stopped', correct_green_2.tStopRefresh)
        trials_ids2.addData('incorrect_red_2.started', incorrect_red_2.tStartRefresh)
        trials_ids2.addData('incorrect_red_2.stopped', incorrect_red_2.tStopRefresh)
        # the Routine "response_feedback" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
        
    # completed trials_in_block repeats of 'trials_ids2'
    ...
    logging.data("utcTime: " + str(time.time()))
    event.clearEvents()
    ids2.addData('press_space_break.started', press_space_break.tStartRefresh)
    ids2.addData('press_space_break.stopped', press_space_break.tStopRefresh)
    # check responses
    if key_resp_break.keys in ['', [], None]:  # No response was made
        key_resp_break.keys = None
    ids2.addData('key_resp_break.keys',key_resp_break.keys)
    if key_resp_break.keys != None:  # we had a response
        ids2.addData('key_resp_break.rt', key_resp_break.rt)
    ids2.addData('key_resp_break.started', key_resp_break.tStartRefresh)
    ids2.addData('key_resp_break.stopped', key_resp_break.tStopRefresh)
    ids2.addData('BREAK.started', BREAK.tStartRefresh)
    ids2.addData('BREAK.stopped', BREAK.tStopRefresh)
    ids2.addData('polygon.started', polygon.tStartRefresh)
    ids2.addData('polygon.stopped', polygon.tStopRefresh)
    ids2.addData('polygon_2.started', polygon_2.tStartRefresh)
    ids2.addData('polygon_2.stopped', polygon_2.tStopRefresh)
    # the Routine "brk" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
    
# completed 100.0 repeats of 'ids2'


# ------Prepare to start Routine "end_break"-------
continueRoutine = True
# update component parameters for each repeat
timer = core.Clock()
key_resp.keys = []
...
_key_resp_allKeys = []
# keep track of which components have finished
end_breakComponents = [text_7, text_8, key_resp, polygon_3, polygon_4]
for thisComponent in end_breakComponents:
    thisComponent.tStart = None
    thisComponent.tStop = None
    thisComponent.tStartRefresh = None
    thisComponent.tStopRefresh = None
...
t = 0
_timeToFirstFrame = win.getFutureFlipTime(clock="now")
end_breakClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
frameN = -1

# -------Run Routine "end_break"-------
while continueRoutine:
    # get current time
    t = end_breakClock.getTime()
    tThisFlip = win.getFutureFlipTime(clock=end_breakClock)
    tThisFlipGlobal = win.getFutureFlipTime(clock=None)
    frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
    # update/draw components on each frame
    ...
    if text_7.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > text_7.tStartRefresh + 180-frameTolerance:
            # keep track of stop time/frame for later
    ...
    # *text_8* updates
    if text_8.status == NOT_STARTED and tThisFlip >= 180-frameTolerance:
        # keep track of start time/frame for later
    ...
    # *key_resp* updates
    waitOnFlip = False
    if key_resp.status == NOT_STARTED and tThisFlip >= 180-frameTolerance:
        # keep track of start time/frame for later
    ...
    if polygon_3.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > polygon_3.tStartRefresh + 179-frameTolerance:
            # keep track of stop time/frame for later
    ...
    if polygon_4.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > polygon_4.tStartRefresh + 179-frameTolerance:
            # keep track of stop time/frame for later
    ...
    if polygon_4.status == STARTED:  # only update if drawing
        polygon_4.setPos((-0.25+0.0014*timer.getTime(), 0.3), log=False)
        polygon_4.setSize((0.0028*timer.getTime(), 0.07), log=False)
...
# the Routine "end_break" was not non-slip safe, so reset the non-slip timer
routineTimer.reset()
